defmodule EctoMaterializedPath do
  defmacro __using__(opts) do
    ...
    quote bind_quoted: [
      ...
      def unquote(:"#{method_namespace}where_depth")(
            query = %Ecto.Query{from: {_, __MODULE__}},
            depth_params
          )
          when is_list(depth_params) do
        EctoMaterializedPath.where_depth(query, depth_params, unquote(:"#{column_name}"))
      end

      def unquote(:"#{method_namespace}arrange")(structs_list) when is_list(structs_list),
        do: EctoMaterializedPath.arrange(structs_list, unquote(:"#{column_name}"))
    end
  end
  ...
  def parent(schema = %{__struct__: struct}, path) do
    ...
    Ecto.Query.from(q in struct, where: q.id == ^parent_id, limit: 1)
  end
  ...
  def subtree(schema = %{__struct__: module, id: id}, column_name) do
    ...
    Ecto.Query.from(q in module,
      where: fragment("? @> ?", field(q, ^column_name), ^path) or q.id == ^id
    )
  end
  ...
  defp do_where_depth(query, [is_bigger_than: ibt], column_name)
       when is_integer(ibt) and ibt > 0 do
    Ecto.Query.from(q in query,
      where: fragment("CARDINALITY(?) > ?", field(q, ^column_name), ^ibt)
    )
  end

  defp do_where_depth(query, [is_bigger_than_or_equal_to: ibtoet], column_name)
       when is_integer(ibtoet) and ibtoet >= 0 do
    Ecto.Query.from(q in query,
      where: fragment("CARDINALITY(?) >= ?", field(q, ^column_name), ^ibtoet)
    )
  end

  defp do_where_depth(query, [is_equal_to: iet], column_name) when is_integer(iet) and iet > 0 do
    Ecto.Query.from(q in query,
      where: fragment("CARDINALITY(?) = ?", field(q, ^column_name), ^iet)
    )
  end

  defp do_where_depth(query, [is_smaller_than_or_equal_to: istoet], column_name)
       when is_integer(istoet) and istoet >= 0 do
    Ecto.Query.from(q in query,
      where: fragment("CARDINALITY(?) <= ?", field(q, ^column_name), ^istoet)
    )
  end

  defp do_where_depth(query, [is_smaller_than: ist], column_name)
       when is_integer(ist) and ist > 0 do
    Ecto.Query.from(q in query,
      where: fragment("CARDINALITY(?) < ?", field(q, ^column_name), ^ist)
    )
  end
  ...
  def build_child(schema = %{__struct__: struct, id: id}, column_name)
      when is_integer(id) and is_atom(column_name) do
  ...
  defp extract_to_resulting_structure(
    ...
    {node_children, node_children_count} =
      ...
      |> Enum.filter(fn possible_children ->
        Map.get(possible_children, column_name) |> List.last() == node.id
      end)
      |> Enum.reduce(
        {[], total_count},
        &extract_to_resulting_structure(&1, &2, nodes_depth_map, next_depth_level, column_name)
      )
